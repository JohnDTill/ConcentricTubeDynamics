...
/*! Find the rotation matrix corresponding to a given quaternion. */
inline Matrix3d quat2rot(Vector4d h){
    double squaredNorm = h.squaredNorm();
    if(squaredNorm==0) return Matrix3d::Identity();
    else return (Matrix3d() <<
        h(0)*h(0)+h(1)*h(1)-h(2)*h(2)-h(3)*h(3),               2*(h(1)*h(2) - h(0)*h(3)),               2*(h(1)*h(3) + h(0)*h(2)),
                      2*(h(1)*h(2) + h(0)*h(3)), h(0)*h(0)-h(1)*h(1)+h(2)*h(2)-h(3)*h(3),               2*(h(2)*h(3) - h(0)*h(1)),
                      2*(h(1)*h(3) - h(0)*h(2)),               2*(h(2)*h(3) + h(0)*h(1)), h(0)*h(0)-h(1)*h(1)-h(2)*h(2)+h(3)*h(3)
                ).finished() / squaredNorm;
}

/*! Find the derivative of a quaternion given an angular velocity specified in the local frame. */
inline Vector4d quatDerivLocal(Vector4d h, Vector3d u){
    return Vector4d(
                           -u(0)*h(1) - u(1)*h(2) - u(2)*h(3),
                u(0)*h(0)             + u(2)*h(2) - u(1)*h(3),
                u(1)*h(0) - u(2)*h(1)             + u(0)*h(3),
