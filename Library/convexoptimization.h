...
static void incrementIterationCounter(int& counter, VectorXd& y, VectorXd& r, int& max_iter){
    if( ++counter > max_iter ){
        std::cout << "ERROR: Max iter of " << max_iter << " exceeded.\n"
                     "Final sum of squares: E = " << r.dot(r) << "\n"
                     "Final residual: f = " << r.transpose() << "\n"
...
template<ObjFunc f>
VectorXd solveSparseLevenbergMarquardt(VectorXd y0,
                                       double sos_tol = 1e-12,
                                       int max_iter = 500,
                                       double damp = 1e-2,
                                       double alm_adptv_coeff = 0.5,
    ...
    VectorXd y = y0;
    int counter = 0;
    SparseMatrix<double> I(y.size(),y.size());
    I.setIdentity();
    SimplicialLDLT< SparseMatrix<double> > sparse_linear_solver;

    while( E > sos_tol ){
        incrementIterationCounter(counter, y, r, max_iter);

        const SparseMatrix<double> J = getSparseJacobian<f>(y,r,sparsity_threshold,incr_scale,incr_floor);
        const SparseMatrix<double> B = J.transpose()*J;
        VectorXd rhs = -J.transpose()*r;

        damp *= alm_adptv_coeff;
        sparse_linear_solver.compute(B + damp*I);

        VectorXd p = sparse_linear_solver.solve(rhs);
        r = f(y + p);
        double Ec = r.squaredNorm();
        ...
        if( !r.hasNaN() && Ec < E ){
            ...
            while( r.hasNaN() || Ec > E ){
                ...
                p = sparse_linear_solver.solve(rhs);
                r = f(y + p);
                Ec = r.squaredNorm();
            }
